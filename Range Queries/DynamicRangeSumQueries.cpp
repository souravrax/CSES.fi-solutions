...
void build(int l, int r, int idx, vector<int>& arr) {
    ...
    tree[idx] = c(tree[idx * 2 + 1], tree[idx * 2 + 2]);
}

int64_t query(int ql, int qr, int l, int r, int idx) {
    if (r < ql || l > qr) {
        return 0;
    }
    if (l >= ql && r <= qr) return tree[idx];
    int mid = l + (r - l) / 2;
    return c(query(ql, qr, l, mid, idx * 2 + 1), 
...
void update(int pos, int val, int l, int r, int idx) {
    ...
    int mid = l + (r - l) / 2;
    if (pos <= mid) {
        update(pos, val, l, mid, idx * 2 + 1);
    }
    else {
        update(pos, val, mid + 1, r, idx * 2 + 2);
    }
    tree[idx] = c(tree[idx * 2 + 1], tree[idx * 2 + 2]);
}
